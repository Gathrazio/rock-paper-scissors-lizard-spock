...
export default function Play ({iconId, updatePlayStats}) {
    ...
    const determineIconAndPositioning = () => {
        ...
        switch (iconId){
                case 1:
                    positionStyles = "sm:top-[7rem] sm:left-[0.5rem] top-[5.5rem] left-[1rem]"
                    return [<FaHandScissors />, positionStyles]
                case 2:
                    positionStyles = ' top-[1.5rem] sm:top-0 sm:left-[10.1rem] left-[6.87rem]'
                    return [<FaHandPaper />, positionStyles]
                case 3:
                    positionStyles = " sm:top-[7rem] sm:right-[0.5rem] top-[5.5rem] right-[1rem]"
                    return [<FaHandRock />, positionStyles]
                case 4:
                    positionStyles = " sm:top-[18.4rem] sm:right-[4rem] top-[12rem] right-[3.3rem]"
                    return [<FaHandLizard />, positionStyles]
                case 5:
                    positionStyles = "sm:top-[18.4rem] sm:left-[4rem] top-[12rem] left-[3.5rem]"
    ...
    return (
        <div className={`rounded-full w-fit sm:p-2 p-5 absolute bg-amber-500 transition ease-in-out delay-5 hover:bg-sky-600 hover:cursor-pointer z-50 ${determineIconAndPositioning()[1]}`} onClick={() => playGame(iconId)}>
            <IconContext.Provider value={{ className: 'w-14 h-14' }}>
